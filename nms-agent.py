...
def main():
    ...
    print(
        f"Auth API key: {'aktif' if a.api_key else 'NONAKTIF (aktifkan AGENT_API_KEY di server!)'}"
    )
    print("-" * 40)
    ...
    while True:
        try:
            ...
            if res.status_code == 200:
                print(
                    f"[OK] {my_ip} | CPU:{cpu:05.1f}% RAM:{ram:05.1f}% | IN:{net_in:05.2f}Mbps OUT:{net_out:05.2f}Mbps"
                )
            elif res.status_code == 403:
                print(
                    "[ERR] API key ditolak server. Samakan AGENT_API_KEY dengan server!"
                )
            ...
            print(
                f"[ERR] Gagal terhubung ke {a.server}. Pastikan NMS Dashboard menyala."
            )
