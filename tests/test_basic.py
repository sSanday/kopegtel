...
class NmsTest(unittest.TestCase):
    ...
    def test_agent_tolak_tanpa_key(self):
        r = self.client.post(
            "/api/agent/report", json={"host": "10.99.99.99", "cpu": 10, "ram": 10}
        )
    ...
    def test_ganti_password_validasi(self):
        r = self.client.post(
            ...
            json={"current_password": "salah", "new_password": "baru12345"},
        )
        self.assertEqual(r.status_code, 400)
    ...
    def test_agent_metrics_null_disk_tidak_500(self):
        conn, c = m.get_db()
        c.execute(
            "INSERT INTO agent_metrics (host,cpu_percent,ram_percent,disk_percent,"
        ...
        conn.close()
        r = self.client.get(
            "/api/agent/metrics", headers={"X-Requested-With": "XMLHttpRequest"}
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_export_sanitasi_host_jahat(self):
        r = self.client.get(
            "/api/export?host=a%0d%0aX-Injected%3A+1&hours=24",
    ...
    def test_delete_event_404_dan_reset_ongoing(self):
        r = self.client.delete(
            "/api/events/999999", headers={"X-Requested-With": "XMLHttpRequest"}
        )
        self.assertEqual(r.status_code, 404)
        m.status_memory["10.99.99.99"] = True
        conn, c = m.get_db()
        ...
        conn.close()
        r = self.client.delete(
            f"/api/events/{eid}", headers={"X-Requested-With": "XMLHttpRequest"}
        )
        self.assertEqual(r.status_code, 200)
        self.assertFalse(m.status_memory.get("10.99.99.99", False))
